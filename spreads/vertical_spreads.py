...
def call_debit_spread(expiry: date, long_call_strike: float, short_call_strike: float):
    ...
    # Long Call
    long_call_plot, = plt.plot([long_call_strike - 10, long_call_strike], [-long_call_price] * 2, 'b--', label=f'Long ${long_call_strike} Call')
    plt.plot([long_call_strike, short_call_strike + 10], [-long_call_price, -long_call_price + strike_spread + 10], 'b--')
    ...
    call_spread_plot, = plt.plot([long_call_strike, short_call_strike], [max_loss, max_gain], 'k-', label='Call Debit Spread')
    plt.plot([long_call_strike - 10, long_call_strike], [max_loss] * 2, 'k-')
    plt.plot([short_call_strike, short_call_strike + 10], [max_gain] * 2, 'k-')
